...
export const customerSignup = catchAsync(async (req, res) => {
    ...
    if (already) throw new ApiError(httpStatus.BAD_REQUEST, "Email already exists");

    // create a temporary customer doc (not fully activated until payment verifies)
    const tempCustomer = await Customer.create({ fullName, shopName, email, phone, password, isActive: false });
...
export const verifySignupPayment = catchAsync(async (req, res) => {
    ...
    await Customer.findByIdAndUpdate(tempCustomerId, { isActive: true });
